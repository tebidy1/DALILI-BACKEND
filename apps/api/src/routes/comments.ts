...
export function registerCommentRoutes(app: FastifyInstance, db: Db, auth: Auth) {
  ...
  app.post('/api/share/:token/comments', async (req, reply) => {
    ...
    if (!parsed.success) {
      return reply.code(400).send({ errorAr: parsed.error.issues[0]?.message ?? 'تعليق غير صالح' })
    }
    const hit = activeShare(db, token)
    if (!hit) return reply.code(404).send({ errorAr: 'الرابط غير موجود أو تم سحبه' })
    return insertComment(reply, hit.guide, parsed.data, false)
  })
  ...
  app.post('/api/guides/:id/comments', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const row = ownedGuideRow(db, user.id, id)
    if (!row) return reply.code(404).send({ errorAr: 'الدليل غير موجود' })
    return insertComment(reply, row, parsed.data, true)
  })
  ...
  app.patch('/api/guides/:id/comments/:cid', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const parsed = zUpdateComment.safeParse(req.body)
    if (!parsed.success) {
      return reply.code(400).send({ errorAr: parsed.error.issues[0]?.message ?? 'تحديث غير صالح' })
    }
    const row = ownedGuideRow(db, user.id, id)
    if (!row) return reply.code(404).send({ errorAr: 'الدليل غير موجود' })
    const target = db
    ...
    if (parsed.data.resolved !== undefined && target.parentId) {
    ...
    if (parsed.data.resolved !== undefined) set.resolved = parsed.data.resolved ? 1 : 0
