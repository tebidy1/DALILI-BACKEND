...
export function registerAssignmentRoutes(app: FastifyInstance, db: Db, auth: Auth) {
  ...
  /** الدليل: مدير المساحة لأي دليل بها، والمنشئ لدليله وحده — غير ذلك 404 صادقة */
  function managedGuideOr404(guideId: string, user: { id: string }, wsId: string, reply: import('fastify').FastifyReply) {
    ...
    if (!guide || guide.workspaceId !== wsId || (!isAdmin && guide.userId !== user.id)) {
      reply.code(404).send({ errorAr: 'الدليل غير موجود' })
      return null
    }
    return guide
  }
  ...
  app.post('/api/guides/:id/assign', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const guide = managedGuideOr404(guideId, user, wsId, reply)
    if (!guide) return guide
    const data = bodyOr400(zCreateAssignment, req.body, reply, 'اختر شخصًا أو فريقًا أو الشركة كلها')
