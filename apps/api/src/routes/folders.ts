...
export function registerFolderRoutes(app: FastifyInstance, db: Db, auth: Auth) {
  ...
  app.post('/api/folders', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const data = bodyOr400(zFolderName, req.body, reply, 'اسم غير صالح')
    if (data === undefined) return data
    const id = nanoid(12)
    const now = new Date().toISOString()
    db.insert(folders)
      .values({ id, userId: user.id, workspaceId: wsOf(user.id, user.email), name: data.name, createdAt: now })
      .run()
    return { id, name: data.name, count: 0, createdAt: now }
  })
  ...
  app.patch('/api/folders/:id', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const data = bodyOr400(zFolderName, req.body, reply, 'اسم غير صالح')
    if (data === undefined) return data
    db.update(folders).set({ name: data.name }).where(eq(folders.id, id)).run()
    const fresh = folderInWsOr404(workspaceId, id)!
    return { id: fresh.id, name: data.name, createdAt: fresh.createdAt }
  })
