...
/** مسار ملف الصوت المشترك: فحص معرّف الخادم ثم حاجز المجلد ثم الوجود — لكل مسار رسالتاه */
function resolveAudioPath(
  filesDir: string,
  fileId: string,
  badIdAr: string,
  missingAr: string,
): { ok: true; path: string } | { ok: false; errorAr: string } {
  if (!SERVER_FILE_ID.test(fileId)) return { ok: false, errorAr: badIdAr }
  const audioPath = path.join(filesDir, path.basename(fileId))
  if (!audioPath.startsWith(filesDir + path.sep) || !fs.existsSync(audioPath)) return { ok: false, errorAr: missingAr }
...
export function registerTranscribeRoute(
  ...
  /** قراءة الصوت واستدعاء المزوّد — الفشل يُسجَّل ويردّ 502 عربي واحد ويقطع المسار */
  async function transcribeOr502(
    ...
    try {
      ...
      return await provider.transcribe(bytes, { mimeType: 'audio/webm' })
    } catch (err) {
      req.log.error(err)
      reply.code(502).send({ errorAr: 'تعذّر التفريغ من مزوّد الصوت — أعد المحاولة لاحقًا' })
      return null
    }
  }
  ...
  app.post('/api/guides/:id/transcribe', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const audio = resolveAudioPath(filesDir, guide.audio.fileId, 'معرّف الصوت غير صالح', 'ملف الصوت غير موجود على الخادم')
    if (!audio.ok) {
      return reply.code(400).send({ errorAr: audio.errorAr })
    }
    const segments = await transcribeOr502(stt, req, reply, audio.path)
    if (!segments) return
  ...
  app.post('/api/guides/:id/transcribe-steps', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    for (const step of guide.steps) {
      ...
      const audio = resolveAudioPath(
        filesDir,
        voice.fileId,
        'معرّف التعليق الصوتي غير صالح',
        'ملف التعليق الصوتي غير موجود على الخادم',
      )
      if (!audio.ok) {
        results.push({ stepId: step.id, ok: false, errorAr: audio.errorAr })
        continue
      }
      const segments = await transcribeOr502(stt, req, reply, audio.path)
      if (!segments) return
