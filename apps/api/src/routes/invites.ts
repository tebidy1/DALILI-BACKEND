...
export function registerInviteRoutes(app: FastifyInstance, db: Db, auth: Auth, publicBase: string) {
  ...
  app.post('/api/team/invites', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const parsed = zInviteCreateReq.safeParse(req.body)
    if (!parsed.success) {
      return reply.code(400).send({ errorAr: parsed.error.issues[0]?.message ?? 'بيانات دعوة غير صالحة' })
    }
    ...
    db.insert(invites)
      .values({ id, workspaceId: ws.id, email, role: parsed.data.role, token, createdAt: now, acceptedAt: null })
      .run()
    return {
      id,
      email,
  ...
  app.post(
    ...
    async (req, reply) => {
    const { token } = req.params as { token: string }
    const parsed = zAcceptInviteReq.safeParse(req.body)
    if (!parsed.success) {
      return reply.code(400).send({ errorAr: parsed.error.issues[0]?.message ?? 'بيانات غير صالحة' })
    }
    const row = db.select().from(invites).where(eq(invites.token, token)).get()
    if (!row) {
      return reply.code(404).send({ errorAr: 'رابط الدعوة غير موجود — اطلب رابطًا جديدًا من مديرك' })
    ...
    if (!user) {
      const id = nanoid(12)
      db.insert(users)
        .values({ id, email, passwordHash: hashPassword(parsed.data.password), createdAt: new Date().toISOString() })
        .run()
      user = db.select().from(users).where(eq(users.id, id)).get()
    } else {
      // حساب معلّق من دعوة قديمة — يستكمل كلمة المرور أخيرًا
      db.update(users).set({ passwordHash: hashPassword(parsed.data.password) }).where(eq(users.id, user.id)).run()
    }
    const already = db
      .select()
