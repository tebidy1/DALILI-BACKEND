...
export function registerTeamRoutes(app: FastifyInstance, db: Db, auth: Auth) {
  function getWorkspace(userId: string) {
    return auth.ensurePersonalWorkspace(userId, '').id
  }
  ...
  app.get('/api/team/teams', { preHandler: auth.requireAuth }, async (req) => {
    ...
    const workspaceId = getWorkspace(user.id)
    return db
      .select({
        id: teams.id,
        name: teams.name,
  ...
  app.post('/api/team/teams', { preHandler: auth.requireAuth }, async (req, reply) => {
    const user = auth.readUser(req)!
    const workspaceId = getWorkspace(user.id)
    if (!requireWorkspaceAdmin(user.id, workspaceId)) {
  ...
  app.post('/api/team', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const parsed = zInviteMemberReq.safeParse(req.body)
    if (!parsed.success) {
  ...
  app.patch('/api/team/:id', { preHandler: auth.requireAuth }, async (req, reply) => {
    ...
    const parsed = zUpdateMemberReq.safeParse(req.body)
    if (!parsed.success) {
      return reply.code(400).send({ errorAr: parsed.error.issues[0]?.message ?? 'بيانات غير صالحة' })
    }
    ...
    if (parsed.data.role !== undefined) updateData.role = normalizeRole(parsed.data.role)
    ...
    if (parsed.data.teamId !== undefined) {
      if (parsed.data.teamId === null) {
        ...
        const team = db
          .select({ id: teams.id })
          .from(teams)
          .where(and(eq(teams.id, parsed.data.teamId), eq(teams.workspaceId, workspaceId)))
          .get()
