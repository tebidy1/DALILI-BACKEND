...
export function makeGuideHelpers(db: Db, auth: Auth, publicBase: string, signer: FileSigner) {
  /** يقرأ الدليل إن كان مملوكًا للمستخدم فعلًا — 404 للغير (عزل لا يكشف الوجود) */
  const ownedGuideOr404 = (userId: string, id: string) => ownedGuideRow(db, userId, id)
  ...
  function runList(user: { id: string; email: string }, q: ListGuidesQuery): ListGuidesDto {
    ...
    const rows = db
      ...
      .offset((page - 1) * limit)
      .all()
    // العدّ بلا ضمّ المشاركات — كل شروط `where` على أعمدة guides أو EXISTS ذاتيّة (PERF-05)
    const total = Number(db.select({ c: sql<number>`count(*)` }).from(guides).where(where).get()?.c ?? 0)
  ...
  return { ownedGuideOr404, visibleGuideOr404, requireNotViewer, shareInfoFor, summaryById, runList }
}
