...
import { fuseRanked, normalizeFa, rankSemantic, relativeCutoff, semanticPassage, vectorToBytes, type EmbeddableGuide } from '@dalili/core'
    ...
    const tx = sqlite.transaction((rows: typeof batch, vecs: Float32Array[]) => {
      rows.forEach((b, j) => {
        ...
        insert.run(
          b.guide.id,
          provider.name,
          sourceHash(provider.name, b.passage),
          v.length,
          vectorToBytes(v),
